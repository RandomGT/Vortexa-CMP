...
class PostDetailViewModel(
     ...
     */
    fun sendComment(content: String, selectedMediaUris: List<Uri> = emptyList()) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                selectedMediaUris.forEach { uri ->
                    homeRepository.uploadPostImage(uri)
                        .onFailure { error ->
                            Log.e(TAG, "sendComment: upload image failed, uri=$uri", error)
                        }
                        .onSuccess { url ->
                            mediaList.add(url)
                        }
                }
                val finalMediaList = if (mediaList.isEmpty()) null else mediaList
